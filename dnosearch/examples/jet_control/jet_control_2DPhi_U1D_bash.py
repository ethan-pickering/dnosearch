...
# -*- coding: utf-8 -*-
"""
Created on Tue May  3 22:35:41 2022
...
def main(seed,iter_num,dim,acq,n_init,epochs,b_layers,t_layers,neurons,init_method,N,print_plots):
    ...
    # Data Paths
    save_path_data = './data/Jet_2DPhi_'+str(acq)+'_Seed_'+str(seed)+'_N'+str(N)+'_iter_'+str(iter_num)+'.mat'
    load_path_data = './data/Jet_2DPhi_'+str(acq)+'_Seed_'+str(seed)+'_N'+str(N)+'_iter_'+str(iter_num-1)+'.mat'
    ...
    x_int_standard = np.linspace(0,16,100) # Static for pt-wise comparisons
    ...
    sio.savemat('./data/Jet_2DPhi_1DU_'+str(acq)+'_Seed_'+str(seed)+'_N'+str(N)+'_iter_'+str(iter_num)+'.mat', {'py_standard':py_standard, 'x_int_standard':x_int_standard, 'Theta':Theta, 'U_opt':U_opt, 'wx':wx, 'ax':ax, 'py':py, 'x_int':x_int, 'Y':Y, 'Mean_Val':Mean_Val, 'Var_Val':Var_Val, 'n_init':n_init, 'N':N, 'seed':seed, 'Theta_test':Theta_test})
    ...
    if print_plots:
        ...
        plt.savefig('./plots/Jet_2DPhi_1DU_'+str(acq)+'_Seed_'+str(seed)+'_N'+str(N)+'_iter_'+str(iter_num)+'.jpg',  dpi=150)
