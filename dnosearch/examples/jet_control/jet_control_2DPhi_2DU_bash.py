...
def main(seed,iter_num,dim,acq,n_init,epochs,b_layers,t_layers,neurons,init_method,N,print_plots):
    ...
    def Theta_to_U(Theta,nsteps,coarse,rank):
        U = np.zeros((np.shape(Theta)[0],nsteps*2))
        phi_0 = -np.pi/2 # original
        wavenumber = 1 # 1
        ...
        for j in range(0,np.shape(Theta)[0]):
            for i in range(0,2):
                U[j,0+(i*nsteps):((i+1)*nsteps)] =  np.sin(wavenumber*(x+phi_0+Theta[j,i]))            
        return U
    ...
    # Data Paths
    save_path_data = './data/Jet_2DPhi_2DU_'+str(acq)+'_Seed_'+str(seed)+'_N'+str(N)+'_iter_'+str(iter_num)+'.mat'
    load_path_data = './data/Jet_2DPhi_2DU_'+str(acq)+'_Seed_'+str(seed)+'_N'+str(N)+'_iter_'+str(iter_num-1)+'.mat'
    ...
    m       = int(nsteps/coarse)*ndim #604*2
    lr      = 0.001
    dim_x   = 1
    activation = "relu"
    ...
    sio.savemat('./data/Jet_2DPhi_2DU_'+str(acq)+'_Seed_'+str(seed)+'_N'+str(N)+'_iter_'+str(iter_num)+'.mat', {'py_standard':py_standard, 'x_int_standard':x_int_standard, 'Theta':Theta, 'U_opt':U_opt, 'wx':wx, 'ax':ax, 'py':py, 'x_int':x_int, 'Y':Y, 'Mean_Val':Mean_Val, 'Var_Val':Var_Val, 'n_init':n_init, 'N':N, 'seed':seed, 'Theta_test':Theta_test})
    ...
    if print_plots:
        ...
        plt.savefig('./plots/Jet_2DPhi_2DU_'+str(acq)+'_Seed_'+str(seed)+'_N'+str(N)+'_iter_'+str(iter_num)+'.jpg',  dpi=150)
